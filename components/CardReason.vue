<template>
    
<div class="w-full h-96 flex justify-between items-center">
    <div class="flex flex-col  w-3/6 gap-20">
        <h2 class="text-4xl font-bold">Paultech is different</h2>
        <div class="flex flex-col gap-5">
        <p class="font bold text-2xl duration-300 hover:text-green-400 cursor-pointer">Commitment to Transparency</p>
        <p class="font bold text-2xl  duration-300 hover:text-green-400 cursor-pointer">Comprehensive and Supportive</p>
        <p class="font bold text-2xl  duration-300 hover:text-green-400 cursor-pointer">Empowering Your Digital Success</p>
        </div>
      

    </div>
    <div class=" w-3/6 flex flex-col gap-10 w-auto">
        <p class="text-3xl text-sky-400"> “Prophet is an insightful consulting firm. Their highly engaged team is easy to work with and provides actionable recommendations. They bring to the table a unique mix of both analytical and creative thinking.”</p>
      
    </div>
</div>
</template>
<script setup>
</script>